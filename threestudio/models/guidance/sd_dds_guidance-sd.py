...
class StableDiffusionDCGuidance(BaseObject):
    ...
    def compute_grad_dds(
        ...
        with torch.no_grad():
            ...
            latent_model_input = torch.cat([latents_noisy_src, latents_noisy_src, latents_noisy_tgt, latents_noisy_tgt], dim=0)
            ...
            source = noise_pred_src_null + self.cfg.guidance_scale * (noise_pred_src_source - noise_pred_src_null)
            target = noise_pred_tgt_null + self.cfg.guidance_scale * (noise_pred_tgt_target - noise_pred_tgt_null)
